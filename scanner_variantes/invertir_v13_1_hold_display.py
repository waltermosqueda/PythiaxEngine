#!/usr/bin/env python3
"""
INVERTIR V13_1 - Variante de Display sobre V13
==============================================
AUTOCONTENIDO: no depende de otros scanners. Solo usa titan_system.

Variante no promovida. Misma logica de V13, mejora solo en display:
  Signal A    - Mean Reversion con regime (hold 7d)
  Signal C5   - Crash + Path Quality + Health Block (hold 4-7d adaptativo)
  Signal D    - Leadership / Tendencia ortogonal (hold 10d)
  Signal E_HW - RS New High sector Hardware/IndustrialTech (hold 15d)

Novedad V13_1:
  - Columna "Hold" en tabla de sizing: muestra dias estimados hasta la salida
  - C5 muestra "4-7d*" por exit adaptativo (+6% en dias 1-4 -> salir antes)
  - Ayuda al operador a no entrar en panico y aguantar el tiempo correcto

Evolucion:
  V7 -> V9 -> V10 -> V11 cap -> V12 signal D -> V13 signal E_HW -> V13_1 hold display

Resultados de referencia (Abr 2020 - Abr 2026, auditados en V23):
  V12 base (3 slots): Sharpe 1.36 | MDD -39.9% | Total +896%
  V13/V13_1 (4 slots): Sharpe 1.62 | MDD -37.0% | Total +1293%

Uso:
  python scanner_variantes/invertir_v13_1_hold_display.py
"""
...
def hold_label(result: ScanResult) -> str:
    """Dias estimados en cartera hasta la salida segun la senal."""
    if result.signal.startswith("A"):
        return f"{A_HOLDING_DAYS}d"
    if result.signal.startswith("C5"):
        # Exit adaptativo: si +6% en dias 1-4 salir antes, sino dia 7
...
def render_sizing_block(results: list[ScanResult], equity_base: float | None) -> str:
    if equity_base is None:
        return (
            ...
            "  O correr el scanner con: python scanner_variantes/invertir_v13_1_hold_display.py --equity TU_CAPITAL\n"
        )
    if not results:
        return ""
    ...
    lines.append("  Hold: dias estimados hasta la salida segun la senal. (*) C5 = exit a +6% si ocurre antes del dia 7.")

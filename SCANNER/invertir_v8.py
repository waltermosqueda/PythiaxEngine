#!/usr/bin/env python3
"""
INVERTIR V8 - DB-Integrated Scanner
===================================

Objetivo:
  Mantener la logica validada de V7, pero ejecutarla mejor dentro del
  ecosistema TITAN:
  1. Usa titan.db en vez de descargar datos en vivo en cada corrida
  2. Reporta cobertura real del universo y frescura de la DB
  3. Agrega contexto de breadth para la decision diaria
  4. Prueba una mejora simple sobre Signal C:
       ROC 10d < -15% + Vol > 2x + RSI < 35

Estado:
  CANDIDATO SUPERADOR. No reemplaza automaticamente a V7.

Evidencia local sobre titan.db (universo amplio disponible en DB):
  V7 base      : 182 trades | WR 67.0% | Sharpe 2.89 | MDD -26.9%
  V8 candidato : 171 trades | WR 68.4% | Sharpe 3.07 | MDD -20.5%
  Walk-forward : 100% positivo en 5w y 7w sobre el universo amplio local

Nota:
  En el universo core historico la mejora no supera a V7 de forma uniforme,
  asi que la confianza es MEDIA. Se crea como archivo nuevo para seguir
  validando sin tocar V7.

Uso:
  python SCANNER/invertir_v8.py
"""
...
# Signal C2: mejora candidata sobre V7
V8_ROC10_MAX = -15.0
V8_VOL_RATIO_MIN = 2.0
V8_C_RSI_MAX = 35.0
...
def signal_c2_crash_volume_oversold(ticker: str, df: pd.DataFrame) -> ScanResult | None:
    if len(df) < 25:
        return None
    ...
    if any(pd.isna(v) for v in [roc10, vol_ratio, curr_rsi, price]):
        return None

    if roc10 >= V8_ROC10_MAX or vol_ratio < V8_VOL_RATIO_MIN or curr_rsi >= V8_C_RSI_MAX:
        return None
    ...
    target = price * 1.05
    risk_pct = (1 - stop / price) * 100
    score = abs(float(roc10)) * float(vol_ratio) * (1 + max(0.0, (V8_C_RSI_MAX - float(curr_rsi))) / 100)

    note = "Candidato V8"
    if get_sector(ticker) == "health":
        note += " | crash healthcare: vigilar"
...
def main() -> None:
    ...
    with TitanDB() as db:
        ...
        for ticker in sorted(t for t in prepared.keys() if t != "SPY"):
            ...
            sig_c2 = signal_c2_crash_volume_oversold(ticker, df)
            if sig_c2 is not None and not any(existing.ticker == ticker for existing in results_a):
                results_c2.append(sig_c2)

        all_results = results_a + results_c2
        all_results.sort(key=lambda item: item.score, reverse=True)

        print("=" * 88)
        print(f"  {MODEL_NAME} - {today.isoformat()}")
        print("=" * 88)
        print(f"  Fuente datos     : titan.db")
        ...
        print("\n" + "=" * 88)
        print("  PASO 1: REGIMEN Y MARKET BREADTH")
        print("=" * 88)
        ...
        print("\n" + "=" * 88)
        print("  PASO 2: SENALES V8")
        print("=" * 88)
        print(f"  Signal A         : RSI<25 + SMA<-10% + Score>30 [igual a V7, con regime]")
        print(f"  Signal C2        : ROC10d<-15% + Vol>2x + RSI<35 [candidato V8, sin regime]")
        print(f"  Total senales    : {len(all_results)} ({len(results_a)} tipo A + {len(results_c2)} tipo C2)")
        ...
        if all_results:
            ...
            for idx, signal in enumerate(top, start=1):
                print(
                    f"  {idx}. {signal.ticker:<6} | {signal.signal:<15} | ${signal.price:>8.2f} | "
            ...
            print("  - C2 endurece la senal de crash: exige que el activo ya este oversold (RSI<35)")
            print("  - En healthcare, las crashes quedan marcadas para vigilancia extra")
        elif regime_safe:
            print("  Regimen seguro, pero sin setups validos. Calidad sobre cantidad.")
        else:
            print("  Regimen peligroso y sin crashes oversold validos. Mejor esperar.")

        print("\n" + "=" * 88)
        print("  VEREDICTO")
        print("=" * 88)
        print("  V8 es un candidato superador de confianza MEDIA.")
        print("  Mejora localmente el universo amplio de titan.db, pero no reemplaza V7")
        print("  hasta completar validacion adicional out-of-sample.")
        print(f"  Ejecutar: python SCANNER/{Path(__file__).name}")

#!/usr/bin/env python3
"""
INVERTIR V11 - Scanner Candidato Superior
========================================
...
Evolucion:
  V7 base -> V9 path quality -> V10 rebound capture -> V11 cap operativa

Cambio de V11:
  V11 no toca la salida de V10.
  V11 endurece el consumo de slot de la pata crash:
    - mantiene la entrada V10
    - acepta solo crashes "preferred":
        score < 85
        vol_ratio < 4.0
    - descarta crashes demasiado violentos que degradan la cartera real

Resultados verificados (Abr 2020 - Abr 2026, revalidados 2026-04-08):
  Base V11 cap:
    - Independiente broad: Sharpe 1.60 vs V10 1.57
    - Independiente core : Sharpe 3.39 vs V10 2.91
    - Cartera broad 3 slots: Sharpe 0.71 vs V10 raw 0.75
    - Cartera core  3 slots: Sharpe 0.88 vs V10 raw 0.62
  Mejora aplicada in place (V14, prioridad por memoria C5_D4):
    - Cartera broad: Sharpe 0.82 vs base 0.71
    - En core no agrega mejora material
...
def load_memory_context(db: TitanDB, regime_label: str) -> list[str]:
    ...
    if regime_label == "SEGURO":
        specs.append((f"{MODEL_NAME}_A_D7", "A / D7"))
        specs.append((f"{MODEL_NAME}_C5_D7", "C5 / D7"))
    else:
        specs.append((f"{MODEL_NAME}_C5_D4", "C5 / D4"))
        specs.append((f"{MODEL_NAME}_C5_D7", "C5 / D7"))
...
def priority_model_name(result: ScanResult) -> str:
    return f"{MODEL_NAME}_A_D7" if result.signal.startswith("A") else f"{MODEL_NAME}_C5_D4"
...
def apply_priority_layer(
    ...
    for result in results:
        profile = profiles.get((priority_model_name(result), regime_label))
        ...
        if expected_return is None:
            result.priority_score = round(float(result.score), 1)
        else:
...
def setup_label(result: ScanResult) -> str:
    return "Rebote (A)" if result.signal.startswith("A") else "Crash (C5)"
...
def load_equity_base() -> float | None:
    """Lee equity_base del estado del gestor. Retorna None si no esta configurado."""
...
def render_sizing_block(results: list[ScanResult], equity_base: float | None) -> str:
    """Bloque visual de simulacion de sizing V15 para cada senal detectada."""
    if equity_base is None:
        return (
            ...
            "  O correr el scanner con: python SCANNER/invertir_v11.py --equity TU_CAPITAL\n"
        )
    if not results:
        return ""

    # Precalcular sizing por resultado y ordenar por notional descendente (mas inversion primero)
...
def render_body(snapshot: Snapshot) -> str:
    results = sorted(
        snapshot.results_a + snapshot.results_c5,
        key=lambda item: (
            float(item.priority_score) if item.priority_score is not None else float(item.score),
        ),
        reverse=True,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="INVERTIR V11 - Scanner activo")
...
def main() -> None:
    ...
    with TitanDB() as db:
        ...
        all_results = apply_priority_layer(db, results_a + results_c5, regime_label, latest_date_str)

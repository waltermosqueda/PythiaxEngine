...
def _build_h7_ticker(snap: dict) -> str:
    """Inner content of .h7-ticker: ticker-lbl + scrolling pk cards for ALL models."""
    league = _dashboard_league(snap)
    all_picks: list[dict] = []
    for row in league:
        ver = str(row.get("version") or "")
    ...
    def _pk_card(p: dict) -> str:
        ...
        return (
            ...
            f"<div class='pk-row1'><span class='pk-sym svb-tk-name'>{_esc(p['sym'])}</span>"
            f"<span class='pk-mod {mod_cls}' style='color:{color}'>{_esc(ver_abbr)}</span>"
            f"<span class='pk-pnl {pct_css} svb-tk-price'>{_esc(price_s)}</span></div>"
            f"<div class='pk-row2'><span class='pk-entry'>\u2014</span>"
...
def render_dashboard_html(html: str, snap: dict, *, verbose: bool = False) -> str:
    ...
    # ── h7 strip: señales activas + ticker (all models) ──────────────────────

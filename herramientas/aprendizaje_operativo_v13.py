#!/usr/bin/env python3
"""
APRENDIZAJE OPERATIVO V13
=========================

Bucle operativo para que Claude:
  - recuerde que predijo V13 cada dia
...
Uso rapido:
  ...
  python herramientas/aprendizaje_operativo_v13.py run --date 2026-04-13
  python herramientas/aprendizaje_operativo_v13.py backfill --from-date 2020-04-09
  python herramientas/aprendizaje_operativo_v13.py report
"""
...
from SCANNER import invertir_v13 as v13
from titan_system.core.database import TitanDB


MODEL_PREFIX = "INVERTIR_V13"
MODEL_VERSION = "v13"
RUNS_DIR = ROOT / "aprendizaje_operativo" / "v13_runs"
REPORTS_DIR = ROOT / "aprendizaje_operativo" / "v13_reports"
A_HORIZONS = (1, 7)
C5_HORIZONS = (1, 4, 7)
D_HORIZONS = (10,)
E_HORIZONS = (15,)

# Reutilizamos la base robusta de V11, pero apuntando al namespace V13.
...
class OperationalLearningV13(base.OperationalLearningV11):
    ...
    def _historical_freshness(self, analyzed_date: str) -> str:
        ...
        staleness = v13.business_days_between(latest_dt, today)
        if staleness <= 1:
            return "AL DIA"
        return f"STALE ({staleness} dias habiles)"

    def build_snapshot(self, requested_date: str | None = None) -> tuple[v13.Snapshot, dict[str, object]]:
        run_started = datetime.now()
        as_of_ts = self.resolve_as_of(requested_date)
        analyzed_date = as_of_ts.date().isoformat()

        regime_safe, regime_info = v13.check_regime(self.prepared["SPY"].loc[:as_of_ts])
        breadth_pct = self._compute_breadth_asof(as_of_ts)
        quality_alerts = self._recent_quality_alerts_asof(as_of_ts)
        memory_context = v13.load_memory_context(self.db, "SEGURO" if regime_safe else "PELIGRO")

        results_a: list[v13.ScanResult] = []
        results_c5: list[v13.ScanResult] = []
        results_d: list[v13.ScanResult] = []
        results_e: list[v13.ScanResult] = []
        blocked_extreme: list[v13.ScanResult] = []
    ...
    def _confidence_for(self, result: v13.ScanResult) -> float:
        if result.signal.startswith("A"):
            raw = (result.score - v13.A_SCORE_MIN) / 50.0
        elif result.signal.startswith("C5"):
            raw = result.score / max(v13.C_SCORE_MAX, 1.0)
        else:
            raw = result.score / 50.0
        bounded = max(0.05, min(0.99, raw))
        return round(float(bounded), 4)

    def _prediction_rows_from_result(self, result: v13.ScanResult, prediction_date: str) -> list[dict[str, Any]]:
        if result.signal.startswith("A"):
            signal_code = "A"
            horizons = A_HORIZONS
        elif result.signal.startswith("C5"):
        ...
        elif result.signal.startswith("D"):
            signal_code = "D"
            horizons = D_HORIZONS
        else:
            signal_code = "E"
    ...
    def record_snapshot(self, snapshot: v13.Snapshot, regime_info: dict[str, object]) -> dict[str, int]:
        ...
        rows: list[dict[str, Any]] = []
        for result in snapshot.results_a + snapshot.results_c5 + snapshot.results_d + snapshot.results_e:
        ...
        return {
            ...
            "signals": len(snapshot.results_a) + len(snapshot.results_c5) + len(snapshot.results_d) + len(snapshot.results_e),
            "blocked": len(snapshot.blocked_extreme),
        }

    def _save_snapshot_artifact(self, snapshot: v13.Snapshot, regime_info: dict[str, object]) -> None:
        artifact = {
            "model_name": MODEL_PREFIX,
            "model_version": MODEL_VERSION,
            "analyzed_date": snapshot.analyzed_date,
            "prediction_for": v13.next_business_day(snapshot.analyzed_date),
            ...
            "results_e": [asdict(result) for result in snapshot.results_e],
            "blocked_extreme": [asdict(result) for result in snapshot.blocked_extreme],
            "runtime_context": base.build_runtime_context(ROOT / "SCANNER" / "invertir_v13.py", self.latest_db_date),
        }
        ...
        self.db.save_model_run_snapshot(
            ...
            regime_label=snapshot.regime_label,
            breadth_pct=snapshot.breadth_pct,
            signal_count=len(snapshot.results_a) + len(snapshot.results_c5) + len(snapshot.results_d) + len(snapshot.results_e),
            snapshot_payload=artifact,
        )

    def _save_regime(self, snapshot: v13.Snapshot, regime_info: dict[str, object]) -> None:
    ...
    def context_memory_rows(self, regime_label: str) -> list[dict[str, Any]]:
        rows = list(super().context_memory_rows(regime_label))

        for model_name, label in [
            (f"{MODEL_PREFIX}_D_D10", "D / D10"),
            (f"{MODEL_PREFIX}_E_D15", "E_HW / D15"),
        ]:
            row = self.db.conn.execute(
                """
                SELECT
    ...
    def daily_summary_text(self, snapshot: v13.Snapshot) -> str:
        status = self.report_status()
        overall = self.report()
        memory_rows = self.context_memory_rows(snapshot.regime_label)
        signals_total = len(snapshot.results_a) + len(snapshot.results_c5) + len(snapshot.results_d) + len(snapshot.results_e)

        lines = [
            "=" * 110,
            f"  RESUMEN DIARIO V13 | {snapshot.analyzed_date}",
            "=" * 110,
            f"  Prediccion para      : {v13.next_business_day(snapshot.analyzed_date)}",
            f"  Regimen actual       : {snapshot.regime_label}",
            f"  Breadth actual       : {snapshot.breadth_pct:.1f}%",
            (
                f"  Oportunidades hoy    : {signals_total} "
                f"({len(snapshot.results_a)} A + {len(snapshot.results_c5)} C5 + "
                f"{len(snapshot.results_d)} D + {len(snapshot.results_e)} E)"
            ),
...
def print_run_summary(snapshot: v13.Snapshot, recorded: dict[str, int], evaluated: dict[str, Any]) -> None:
    print("=" * 90)
    print(f"  APRENDIZAJE OPERATIVO V13 | {snapshot.analyzed_date}")
    print("=" * 90)
    ...
    print(f"  Senales E        : {len(snapshot.results_e)}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bucle de aprendizaje operativo para INVERTIR V13")
    ...
    sub.add_parser("report", help="Resumen de memoria y metricas acumuladas V13")
    ...
    recompute_parser = sub.add_parser(
        "recompute-outcomes",
        help="Recalcular outcomes V13 con retorno operable (open siguiente -> close target)",
    )
...
def run_backfill(db: TitanDB, from_date: str, to_date: str | None) -> None:
    ...
    engine.refresh_model_metrics()
    print("=" * 90)
    print(f"  BACKFILL V13 | {dates[0]} -> {dates[-1]}")
    print("=" * 90)

...
def test_build_dashboard_history_report_ignores_optional_missing_snapshot_history(monkeypatch) -> None:
    ...
    class FakeConn:
        ...
        def execute(self, query, params=()):
            if "SELECT DISTINCT prediction_date" in query:
                return FakeRows([(day,) for day in window_dates])
            if "SELECT DISTINCT p.target_date" in query:
                return FakeRows([(day,) for day in window_dates])
            if "SELECT DISTINCT date" in query and "FROM regimes" in query:
                return FakeRows([(day,) for day in window_dates])
            raise AssertionError(query)
...
def test_build_dashboard_history_report_rejects_missing_prediction_days(monkeypatch) -> None:
    class FakeRows:
        def __init__(self, rows):
            self._rows = rows
    ...
    class FakeConn:
        ...
        def execute(self, query, params=()):
            if "SELECT DISTINCT prediction_date" in query:
                return FakeRows([("2026-03-19",), ("2026-03-20",)])
            if "SELECT DISTINCT p.target_date" in query:
                return FakeRows([("2026-03-18",), ("2026-03-19",), ("2026-03-20",)])
            if "SELECT DISTINCT date" in query and "FROM regimes" in query:
                return FakeRows([("2026-03-18",), ("2026-03-19",), ("2026-03-20",)])
            raise AssertionError(query)

    window_dates = ["2026-03-18", "2026-03-19", "2026-03-20"]

    monkeypatch.setattr(
        auto_actualizar,
        "dashboard_history_window_dates",
        lambda fecha_base, min_market_days=90: window_dates,
    )
    monkeypatch.setattr(
        auto_actualizar,
        "expected_monitored_snapshot_entries",
    ...
    monkeypatch.setattr(
        ...
        lambda con, entry, start_date, end_date: {
            "prediction_days": len(window_dates),
            "total_prediction_rows": 300,
            "latest_prediction_date": end_date,
        },
    )

    report = auto_actualizar.build_dashboard_history_report(date(2026, 3, 20), min_market_days=len(window_dates))

    assert report["history_complete"] is False
    assert report["window_coverage"]["predictions"]["missing_days"] == ["2026-03-18"]
...
def test_ensure_minimum_dashboard_history_bootstraps_sparse_cloud_history(monkeypatch) -> None:
    ...
    reports = iter(
        [
            ...
            },
            {
                ...
                "regimes_recent": 90,
                "window_coverage": {
                    "predictions": {"missing_days": ["2026-03-18"]},
    ...
    monkeypatch.setattr(
        auto_actualizar,
        "backfill_optional_observed_history",
        lambda from_date, fecha_base: calls.append(f"optional_observed:{from_date}"),
    )
    monkeypatch.setattr(
        auto_actualizar,
        "backfill_optional_legacy_history",
        lambda from_date, fecha_base: calls.append(f"optional_legacy:{from_date}"),
    )
    monkeypatch.setattr(
        auto_actualizar,
        "recompute_required_outcomes",
    ...
    monkeypatch.setattr(
        auto_actualizar,
        "recompute_optional_observed_outcomes",
        lambda fecha_base: calls.append("optional_observed_outcomes"),
    )
    monkeypatch.setattr(
        auto_actualizar,
        "recompute_optional_legacy_outcomes",
        lambda fecha_base: calls.append("optional_legacy_outcomes"),
    )
    ...
    assert calls == [
        "required:2026-01-01",
        "required_outcomes",
        "optional_observed:2026-01-01",
        "optional_observed_outcomes",
        "optional_legacy:2026-01-01",
        "optional_legacy_outcomes",
    ]
...
def test_ensure_minimum_dashboard_history_returns_after_observed_optional_repair(monkeypatch) -> None:
    calls: list[str] = []
    reports = iter(
        [
            {
